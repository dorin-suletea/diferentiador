...
func NewTextArrayFromString(content string) *TextArray {
	grid := NewTextArray()
	grid.SetText(content)
	return grid
}

// TextArray is a monospaced grid of characters.
...
func (t *TextArray) MinSize() fyne.Size {
...
func (t *TextArray) Resize(size fyne.Size) {
...
func (t *TextArray) Row(row int) TextGridRow {
...
func (t *TextArray) RowText(row int) string {
	rowData := t.Row(row)
...
func (t *TextArray) SetRowStyle(row int, style TextGridStyle) {
	if row < 0 {
		return
	}
	for len(t.Rows) <= row {
...
func (t *TextArray) SetCell(row, col int, cell TextGridCell) {
	if row < 0 || col < 0 {
		return
	}
...
func (t *TextArray) SetRune(row, col int, r rune) {
	if row < 0 || col < 0 {
		return
	}
	t.ensureCells(row, col)
...
func (t *TextArray) SetStyle(row, col int, style TextGridStyle) {
	if row < 0 || col < 0 {
		return
	}
	t.ensureCells(row, col)
...
func (t *TextArray) SetStyleRange(startRow, startCol, endRow, endCol int, style TextGridStyle) {
...
func (t *TextArray) ensureCells(row, col int) {
	for len(t.Rows) <= row {
		t.Rows = append(t.Rows, TextGridRow{})
	}
	data := t.Rows[row]
...
// NewTextGridFromString creates a new TextGrid widget with the specified string content.
func NewTextGridFromString(content string) *TextArray {
	grid := NewTextGrid()
	grid.SetText(content)
	return grid
}
