...
var _ internal.CacheListener = (*ChangedFilesWidget)(nil)
...
func NewChangedFilesWidget(data *ChangedFileCache, diffWidget *DiffWidget) *ChangedFilesWidget {
	list := newFilesStatusList(data.GetAll())
	scroll := container.NewScroll(list)

	ret := &ChangedFilesWidget{internal.NewFocusComponent(scroll), scroll, list, 0, data, diffWidget}
	if data.Len() != 0 {
		ret.selectItem(0)
	}
...
func (dw *ChangedFilesWidget) OnCacheRefreshed() {
	newData := dw.data.GetAll()
